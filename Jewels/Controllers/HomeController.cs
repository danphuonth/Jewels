...
namespace Jewels.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult TraCuu(string searchString)
        {
            ...
            IEnumerable<DonHang> items = db.DonHangs.Include(d => d.KhuyenMai).Include(d => d.TrangThaiDH).Include(d => d.HinhThucTT).OrderByDescending(x => x.DonHangID);
            if (!string.IsNullOrEmpty(searchString))
            {
                items = db.DonHangs.Where(x => x.DienThoai.Contains(searchString));
                return View(items);
            }
            else
            {
